...
/**
 * Creates metadata for domain entity ITDs (annotated with {@link NodeEntity}.
 * Based on
 * org.springframework.roo.addon.layers.repository.mongo.MongoEntityMetadata.
 * 
 * @author jccastrejon
 * 
 */
public class NodeEntityMetadata extends AbstractItdTypeDetailsProvidingMetadataItem {

    private static final String PROVIDES_TYPE_STRING = NodeEntityMetadata.class.getName();
    ...
    private NodeEntityMetadata parent;
     ...
     * @param parent
     * @param idType
     *            the type of the entity's identifier field (required)
     * @param governorMemberDetails
     *            the member details of the entity
     */
    public NodeEntityMetadata(final String identifier, final JavaType aspectName,
            final PhysicalTypeMetadata governorPhysicalTypeMetadata, NodeEntityMetadata parent, final JavaType idType,
    ...
    private MethodMetadataBuilder getIdentifierMutator() {
        if (parent != null) {
            ...
            if (parentIdMutator != null && parentIdMutator.getParameterTypes().get(0).getJavaType().equals(idType)) {
